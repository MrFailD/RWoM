...
namespace TorannMagic.Golems
{
    ...
    public class Verb_NullCleave : Verb_MeleeAttack
    {
        ...
        protected override DamageWorker.DamageResult ApplyMeleeDamageToTarget(LocalTargetInfo target)
        {
            ...
            DamageInfo dinfo = new DamageInfo(TMDamageDefOf.DamageDefOf.TM_Shadow, (int)(tool.power), 0, (float)-1, CasterPawn, null, null, DamageInfo.SourceCategory.ThingOrUnknown);
            damageResult.hitThing = target.Thing;
            damageResult.totalDamageDealt = Mathf.Min((float)target.Thing.HitPoints, dinfo.Amount);
            float angle = (Quaternion.AngleAxis(90, Vector3.up) * TM_Calc.GetVector(CasterPawn.Position, target.Thing.Position)).ToAngleFlat();
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                {
                    DamageInfo dinfo2 = new DamageInfo(TMDamageDefOf.DamageDefOf.TM_Shadow, (int)(tool.power * .6f), 0, (float)-1, CasterPawn, null, null, DamageInfo.SourceCategory.ThingOrUnknown);
