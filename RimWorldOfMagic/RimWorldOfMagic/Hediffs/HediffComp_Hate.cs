    ...
    public class HediffComp_Hate : HediffComp
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            if (flag)
            {
                ...
                {
                    Hediff hediff = Pawn.health.hediffSet.GetFirstHediffOfDef(TorannMagicDefOf.TM_Artifact_HateBoostHD);
                    float maxSev = 100;
                    float minSev = 0;
                    if (hediff != null)
                    {
                    ...
                    if ((lastHateTick + 1200) < Find.TickManager.TicksGame && parent.Severity > (25f + minSev + (Pawn.health.hediffSet.PainTotal * 50)))
                    {
                        severityAdjustment -= Rand.Range(.1f, .2f);
                    }

                    if(parent.Severity < (20f + minSev + (Pawn.health.hediffSet.PainTotal * 50)))
                    {                        
            ...
            for (int i = 0; i < shroudCells.Count(); i++)
            {
                ...
                for (int j = 0; j < cellList.Count; j++)
                {
                    ...
                    if (projectile != null)
                    {
                        ...
                        if (weaponDamageMultiplier > 0 && launcher != null && launcher != Pawn && (projectileOrigin - Pawn.DrawPos).MagnitudeHorizontal() > 8 && Rand.Chance(.3f + (.06f * hateVer)))
                        {                            
