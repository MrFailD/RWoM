        ...
        public void TickAction()
        {
            ...
            foreach (Hediff current in hds)
            {
                if (verVal >= 1)
                {
                    ...
                    {
                        current.Severity -= (.001f * parent.Severity * (1f + (.1f * pwrVal)));
                    }
                }
                if (verVal >= 2)
                {
                    ...
                    if (hc_i != null)
                    {
                        current.Severity -= (hc_i.Props.severityPerDayNotImmune / 100f) * parent.Severity;
                    }
                }
            }
            ...
            foreach (Hediff_Injury current in pawn.health.hediffSet.hediffs.OfType<Hediff_Injury>())
            {
                if (current.CanHealNaturally())
                {
                    ...
                    current.Heal((Rand.Range(.2f, .3f) + healBleedingBonus) * parent.Severity * (1f + (.1f * pwrVal))); //140-200 applications = 14 - 39 healing for every wound                            
                }        
                else if(verVal >= 3)
                {
                    current.Heal(Rand.Range(.008f, .012f) * parent.Severity * (1f + (.1f * pwrVal))); //.56 - 1.56 permanent injury healing for every wound
                }
            }
        }
    }
