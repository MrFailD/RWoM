...
namespace TorannMagic
{
    ...
    public class Building_TMElementalRift_Defenders : Building
    {
        ...
        public override void Destroy(DestroyMode mode = DestroyMode.Vanish)
        {
            ...
            {
                if (!pList[i].DestroyedOrNull() && !pList[i].Dead && pList[i].def.defName.Contains("Elemental") && pList[i].Faction == Faction)
                {
            ...
            while(elementalPawns.Count > 0)
            {
                ...
                FleckMaker.ThrowSmoke(elementalPawns[0].DrawPos, Map, 1.2f);
                    ...
                    else
                    {
                        ...
                        if (newPawn.Faction != null && newPawn.Faction != Faction.OfPlayer)
                        {
                            ...
                            if (flag4)
                            {
                                ...
                                LordJob_DefendBase lordJob = new LordJob_DefendBase(newPawn.Faction, Position, 0);
