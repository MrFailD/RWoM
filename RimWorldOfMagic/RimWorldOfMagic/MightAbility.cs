...
namespace TorannMagic
{
    ...
    {        
        ...
        public CompAbilityUserMight MightUser
        {
            get
            {
                return MightUserUtility.GetMightUser(Pawn);
            }
        }
		...
		{
            abilityUser = (abilityData.Pawn.AllComps.FirstOrDefault((ThingComp x) => x.GetType() == abilityData.AbilityClass) as CompAbilityUserMight);
        }
        ...
        public override void PostAbilityAttempt()  
        {
            ...
            if (!Pawn.IsColonist && ModOptions.Settings.Instance.AIAggressiveCasting)// for AI
            {
            ...
            if (flag)
            {
                ...
                if (mightDef.abilitiesRemovedWhenUsed != null && mightDef.abilitiesRemovedWhenUsed.Count > 0)
                {
                    ...
                    {
                        MightUser.RemovePawnAbility(rem);
                    }
                }
            }
        }
        ...
        {
            MightUser.TryAddPawnAbility(mightDef.chainedAbility);
            bool expires = false;
            int expireTicks = -1;
        ...
        public override bool CanCastPowerCheck(AbilityContext context, out string reason)
        {
            ...
            if (flag)
            {
                ...
                bool flag1 = Def != null && (tmAbilityDef = (Def as TMAbilityDef)) != null;
                if (flag1)
                {
                    ...
                    if (flag4)
                    {
                        ...
                        if (flagInspiration)
                        {
                            if (Pawn.mindState.inspirationHandler != null && Pawn.mindState.inspirationHandler.CurStateDef == mightDef.requiredInspiration)
                            {                                
                                //passes hediff requirements
                            }
                            else
                    ...
                    mightDef == TorannMagicDefOf.TM_Buckshot || mightDef == TorannMagicDefOf.TM_BreachingCharge))
                {
                    if (MightUser.Pawn.equipment != null && MightUser.Pawn.equipment.Primary != null)
                    {
                        reason = "TM_MustHaveWeaponType".Translate(
                                Pawn.LabelShort,
                                Pawn.equipment.Primary.def.label,
                                "specialized weapon"
                            );                        
                    }
                    else
