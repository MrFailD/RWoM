        ...
        protected override bool TryCastShot()
        {
            ...
            if (pawn != null && !pawn.Downed)
            {
                ...
                else
                {
                    ...
                    {
                        if(allPawns[i].Faction != null && allPawns[i].HostileTo(CasterPawn.Faction) && allPawns[i].CurJob != null && allPawns[i].CurJob.targetA != null && allPawns[i].CurJob.targetA.Thing == CasterPawn)
                        {
                            allPawns[i].jobs.EndCurrentJob(Verse.AI.JobCondition.InterruptForced, true);
                        }
                    }
                    if (CasterPawn.GetCompAbilityUserMagic()?.MagicData.MagicPowerSkill_Cantrips.FirstOrDefault((MagicPowerSkill x) => x.label == "TM_Cantrips_ver").level >= 12)
                    {
                        HealthUtility.AdjustSeverity(pawn, TorannMagicDefOf.TM_InvisibilityHD, 5f);
                        TM_Action.DoAction_HealPawn(CasterPawn, CasterPawn, 3, 7);
                    }
                }                
                arg_40_0 = true;
            }
