...
namespace TorannMagic
{
    ...
    public class Projectile_ReverseShield : Projectile_AbilityBase
    {
        ...
        public override void ExposeData()
        {
            ...
            Scribe_Collections.Look<IntVec3>(ref shieldBlockCells, "shieldBlockCells", LookMode.Value);
            Scribe_Collections.Look<IntVec3>(ref shieldInternalCells, "shieldInternalCells", LookMode.Value);
        }


        private void Initialize()
        {
            shieldInternalCells = GenRadial.RadialCellsAround(Position, radius - 1, true).ToList();
            shieldBlockCells = GenRadial.RadialCellsAround(Position, radius + 1, true).Except(shieldInternalCells).ToList();
        ...
        {            
            ...
            if(Find.TickManager.TicksGame % tickFrequency == 0 && shieldInternalCells != null && shieldBlockCells != null && Map != null)
            {
                List<Thing> tmpProjs = Map.listerThings.AllThings.Where(t => t is Projectile).ToList();
                if (tmpProjs != null)
                {
                    foreach (Thing t in tmpProjs)
                    {
                        ...
                        if (p != null && !p.Launcher.DestroyedOrNull() && shieldInternalCells.Contains(p.Launcher.Position) && shieldBlockCells.Contains(p.Position))
                        {
                            ...
                            TM_MoteMaker.ThrowGenericMote(TorannMagicDefOf.Mote_Shadow, p.DrawPos, Map, Rand.Range(.6f, 1f), .1f, 0f, .2f, Rand.Range(-100, 100), p.def.projectile.speed / 4f, dirAngle, Rand.Range(0, 360));
                            p.Destroy();
                        }
                    }
                }
